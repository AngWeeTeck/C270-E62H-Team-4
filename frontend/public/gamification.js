...
try {
  authenticatedUser = JSON.parse(localStorage.getItem("threadquest_auth_user") || "null");
} catch (_error) {
  authenticatedUser = null;
}

const AUTHOR = String(authenticatedUser?.username || "").trim().toLowerCase();
...
async function loadPlayer() {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}`);
...
async function resetProgress() {
  const response = await authFetch(`${API_URL}/${AUTHOR}/reset`, {
    method: "POST"
  });
...
async function rewardPlayer(action, message) {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}/reward`, {
      method: "POST",
      headers: {
...
async function buyItem(item, amount) {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}/spend`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
async function applyItem(item) {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}/apply`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
async function runDevApiAction(action, message) {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}/dev/${action}`, {
      method: "POST"
    });
...
async function claimDailyReward() {
  try {
    const response = await authFetch(`${API_URL}/${AUTHOR}/daily-reward`, {
      method: "POST"
    });
