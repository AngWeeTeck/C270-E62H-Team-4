...
🛠️  Developer Tools Available:
  ...
  const handleCreateThread = async (newThread) => {
    ...
    if (window.__communityHub?.announceActivity) {
      window.__communityHub.announceActivity('thread-created', { title: newThread.title || 'a new thread' });
    }
  };
  ...
  const handleCommunityAction = (communityName, joined) => {
    if (window.__communityHub?.announceActivity) {
      window.__communityHub.announceActivity('community-joined', { communityName, joined });
    }
  };
      ...
      <section className="section-grid">
        ...
        <div className="card card-feed">
          <div className="card-header">
            <div>
              <h2>Community hub</h2>
              <p>Track updates and discover study communities.</p>
            </div>
          </div>
          <CommunityHub onCommunityAction={handleCommunityAction} />
        </div>
      </section>
